    ...
    Y.all('[data-mixcloud-api-url]').each(function (embed) {
        ...
        if (api_url&&!embed.hasClass('inited')) {
            ...
            $.getJSON(api_url + '?callback=', function (data) {
                ...
                content.append('<div class="custom-mixcloud-widget"><div class="track-art" style="background: #333 url('+data.pictures.medium+') no-repeat;background-size: cover"></div><a class="text-info clear">' +
...
if (!window_loaded && Y.one('.embed-block[data-block-json*="mixcloud.com"]')) {
    ...
    Y.all('.embed-block[data-block-json*="mixcloud.com"]').each(function (item) {
        ...
        console.log(feed);
        if (feed) {
            item.setAttribute('data-mixcloud-api-url', 'https://api.mixcloud.com/' + feed).addClass(slugify(feed)+'-mix-item');
            content.empty();
        }
    });
    activateMixcloudThings();
