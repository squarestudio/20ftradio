window.Template.Controllers.CastController = function (element) {
    ...
    function initialize() {
        ...
        var events = Array.prototype.splice.call(document.querySelectorAll('.eventlist--upcoming .event-item'));
        if(events.length){
    ...
    function initShoutCast() {
        if (shoutCastUrl) {
            ...
            if (!shoutcastPlayer) {
                shoutcastPlayer = Y.Node.create('<video id="shoutcastPlayer" title="20FT Radio" class="stream-player" poster="https://www.20ftradio.net/assets/icon.png" preload playsinline -webkit-playsinline name="media"><source src="' + shoutCastUrl + '" type="audio/mpeg"></video>');
            }
