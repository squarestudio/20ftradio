...
Y.use('node', 'squarespace-gallery-ng', function (Y) {
    ...
    Y.once('appready', function () {
        ...
        Y.all('a.home-link').setAttribute('href', window.serverURL + window.cordovaURL);
        window.ajaxL = new AjaxLoader({
            sqsController: true,
            timeout: 6000,
    ...
    window.Site = Singleton.create({
        ...
        ready: function () {
            this.listeners = [];
            Y.once('appready', this.initialize.bind(this));
        },
        ...
        _setHeaderValues: function () {
            if (Y.one('#topBar')) {
                ...
                if (Y.one('#castDiv')) {
                    if (Y.one('body').hasClass('cordova-app')) {
                        Y.one('#castDiv').setStyle('paddingTop', topBarHeight);
                    } else {
        ...
        _checkSidebarHeight: function (proj) {
            ...
            if (Y.config.win.innerWidth > 1000 && metaHeight + Y.one('#topBar .nav-container').get('offsetHeight') > Y.config.win.innerHeight) {
        ...
        jumpDown: function (e) {
            ...
            if (Y.one('#topBar')) {
                if (this.currImg < proj.all("img").size()) {
                    ...
                    newPos = 5 + newPos - Y.one('#topBar .nav-container').get('offsetHeight') - parseInt(proj.one('.gallery').getStyle('marginTop'));
                    this.animate('body', {
                        to: {
                            scroll: [0, newPos]
                        },
                    ...
                    newPos = newPos - Y.one('#topBar .nav-container').get('offsetHeight') - parseInt(proj.one('.gallery').getStyle('marginTop'));
                    this.animate('body', {
                        to: {
                            scroll: [0, newPos]
                        },
...
function localizeAndBuildDates() {
    var body = Y.one('body');
    Y.use('node', function () {
        ...
        Y.all('[date-format]').each(function (time) {
            ...
            if (format === 'ddd'){
                format = 'dddd';
                time.setAttribute('date-format', 'dddd')
            }
            var textContent = moment(value).format(format);
            if (format === 'dddd' && isToday(value)) {
    ...
    if (body.one('.new-events-schedule')) {
        ...
        if (new_schedule.one('.date-container')) {
            ...
            new_schedule.all('.event-item').each(function (item) {
                item.append('<div class="event-schedule-wrapper"><a href="#" class="schedule-event">' +
                    '<svg class="add" viewBox="0 0 286.376 286.376">' +
                    '<g>' +
                    ...
                    '</g>' +
                    '</svg><svg class="calendar" viewBox="0 0 425 425"><g><path d="M293.333,45V20h-30v25H161.667V20h-30v25H0v360h425V45H293.333z M131.667,75v25h30V75h101.666v20h30V75H395v50H30V75H131.667z M30,375V155h365v220H30z"/><polygon points="203.28,277.107 165.686,239.513 144.473,260.726 203.28,319.534 280.527,242.286 259.314,221.073"/></g></svg></a></div>')
            });
...
Y.config.win.Squarespace.onInitialize(Y, function () {
    var body = Y.one('body');
    localizeAndBuildDates();
    if (window.pageYOffset > 100) {
        body.addClass('track-fixed');
    }
    Y.use('node', function () {
        moment.locale(navigator.language);
        Y.all('[date-format]').each(function (time) {
    ...
    if (location.pathname.indexOf('mobile-app') === -1 && location.pathname.indexOf('www/index.html') === -1) {
...
document.addEventListener('scroll', function (e) {
    if (window.pageYOffset > 100) {
        Y.one('body').addClass('track-fixed');
    } else {
        Y.one('body').removeClass('track-fixed');
    }
});