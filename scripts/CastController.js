window.Template.Controllers.CastController = function (element) {
    ...
    function initialize() {
        ...
        if(events.length){
            events.forEach(function (event) {
                console.log(event, currentTime,new Date(event.getAttribute('data-end-date') + siteTimezoneOffset + userTimezoneOffset).getTime())
    ...
    function initShoutCast() {
        if (shoutCastUrl) {
            ...
            if (!shoutcastPlayer) {
                YshoutcastPlayer = Y.Node.create('<video id="shoutcastPlayer" src="' + shoutCastUrl +'" title="20FT Radio" class="stream-player"  poster="https://www.20ftradio.net/assets/icon.png" preload="auto" playsinline -webkit-playsinline name="media" crossorigin="anonymous"></video>');
            }
